...
//
// <public-path> is everything after the domain, starting with "img/" or
// "video/" to match the site route that will serve it back:
//   pnpm run publish-image ./hero.webp img/blogs/atom-announcement/hero.webp
//   pnpm run publish-image ./bg.mp4 video/hero-background.mp4
...
// live in the bucket. Keep in sync with src/pages/img/[...path].ts and
// src/pages/video/[...path].ts.
const ROUTES = {
  img: { keyPrefix: "absmach-website" },
  video: { keyPrefix: "absmach-website/video" },
};
...
if (!localFile || !publicPath) {
  console.error(
    "Usage: pnpm run publish-image <local-file> <public-path>\n" +
      "Example: pnpm run publish-image ./hero.webp img/blogs/atom-announcement/hero.webp",
  );
  process.exit(1);
}
...
if (!route || restParts.length === 0) {
  console.error(
    `Destination must start with "img/" or "video/" and include a path, got: ${destKey}`,
  );
  process.exit(1);
}
